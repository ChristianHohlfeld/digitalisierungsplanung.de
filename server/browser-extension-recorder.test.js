...
test("product recorder uses the browser extension instead of developer localhost setup", () => {
  ...
  assert.doesNotMatch(html, /npm run recorder:agent/i);
  assert.doesNotMatch(html, /127\.0\.0\.1:8799/);
  assert.doesNotMatch(html, /Local Recorder Agent/i);
  assert.match(html, /ZUSTAND_EXTENSION_COMMAND/);
  assert.match(html, /Desktop Recorder/);
  assert.match(html, /Aufnahme auf Desktop verfügbar/);
});
