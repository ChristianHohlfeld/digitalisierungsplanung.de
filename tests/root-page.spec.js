...
test.describe("Root demo export", () => {
  ...
  test("serves the single Zustand demo at root @smoke", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Digitalisierungsplanung.de", exact: true })).toBeVisible();
    await expect(page.getByText("Der visuelle Editor").first()).toBeVisible();
    ...
    const expectDemoStep = async (stateId, title) => {
      await expect(page.locator("#statePill")).toHaveText(stateId);
